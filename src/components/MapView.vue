...
function dotHtml(color) {
  return `<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2px solid white;box-shadow:0 1px 3px rgba(0,0,0,.4)"></div>`
}
...
function addMarker(station) {
  ...
  const icon = L.divIcon({ className: '', html: dotHtml(getMarkerColor(station)), iconSize: [14, 14], iconAnchor: [7, 7] })
...
function updateMarker(sno, station) {
  ...
  if (!m) return
  m.setIcon(L.divIcon({ className: '', html: dotHtml(getMarkerColor(station)), iconSize: [14, 14], iconAnchor: [7, 7] }))
